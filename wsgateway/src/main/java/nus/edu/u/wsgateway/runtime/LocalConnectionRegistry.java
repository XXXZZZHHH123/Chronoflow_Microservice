...
public class LocalConnectionRegistry {
    ...
    public Flux<String> stream(String userId) {
        Channel ch = channels.computeIfAbsent(userId, k -> {
        ...
        return ch.sink.asFlux()
                ...
                .doFinally(sig -> {
                    ...
                    log.info("[WS] userId={} disconnected (signal={}), subscribers={}", userId, sig, n);
                    if (n <= 0) {
    ...
    public boolean pushJson(String userId, String json) {
        ...
        switch (r) {
            case FAIL_OVERFLOW -> log.warn("[WS] buffer overflow for userId={} (drop message)", userId);
