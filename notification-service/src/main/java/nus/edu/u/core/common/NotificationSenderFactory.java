...
/**
 * Factory that dynamically selects the correct NotificationSender implementation
 * based on the provided NotificationChannel.
 */
@Component
public class NotificationSenderFactory {
    ...
    public NotificationSenderFactory(List<NotificationSender> senders) {
        this.registry = senders.stream().collect(Collectors.toMap(
                sender -> Arrays.stream(NotificationChannel.values())
                        .filter(sender::supports)
                        .findFirst()
                        .orElseThrow(() -> new IllegalStateException(
                                sender.getClass().getSimpleName() + " supports no channel")),
                Function.identity()
        ));
    }
