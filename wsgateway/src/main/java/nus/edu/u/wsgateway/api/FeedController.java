...
public class FeedController {
    ...
    public Mono<ResponseEntity<Map<String, Object>>> push(@RequestBody WsPushRequestDTO req) {
        return feedService.createOrTouchAndPush(req)
                .map(doc -> {
                    ...
                    if (doc.getId() != null) body.put("id", doc.getId()); // avoid null in Map.of
    ...
    public Mono<Map<String, Object>> markSeen(@RequestBody MarkSeenRequestDTO req) {
        return feedService.markSeen(req.getUserId(), req.getNotificationIds())
                .flatMap(updated ->
                        feedService.unreadCount(req.getUserId())
                                .map(unread -> {
