        ...
        pubSubTemplate.subscribe(
                ...
                message -> {
                    try {
                        ...
                        EmailPubSubMessage m =
                                objectMapper.readValue(data, EmailPubSubMessage.class);

                        if (m.getTo() == null || m.getEventId() == null) {
                            log.warn(
                                    "[PUBSUB] ⚠️ Skipping invalid message: missing required fields");
                            message.ack(); // 👈 ACK even for bad messages — don’t requeue poison
                            // data
                            return;
                        }
                        ...
                        NotificationRequestDTO req =
                                NotificationRequestDTO.builder()
                                        ...
                                        .templateId(
                                                Optional.ofNullable(m.getTemplateId())
                                                        .orElse("member-invite"))
                                        .variables(
                                                m.getVariables() != null
                                                        ? m.getVariables()
                                                        : Map.of())
                                        .locale(
                                                Optional.ofNullable(m.getLocale())
                        ...
                        log.info(
                                "[PUBSUB] ✅ Email notification processed. to={} eventId={} result={}",
                                m.getTo(),
                                m.getEventId(),
                                result);
